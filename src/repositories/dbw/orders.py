"""Репозиторий: Заказы DBW — поля строго по WB API v3 Order schema."""
...
# 32767 // 26 cols = 1260 rows per batch
CHUNK_SIZE = 1260
...
class DbwOrdersRepository:
    ...
    async def upsert_many(self, orders: list[dict]) -> int:
        ...
        for i in range(0, len(rows), CHUNK_SIZE):
            ...
            stmt = insert(DbwOrder).values(chunk)
    ...
    async def get_max_created_at(self) -> datetime | None:
        result = await self._session.execute(select(func.max(DbwOrder.created_at)))
    ...
    async def get_filtered(
        ...
        if date_from:
            query = query.where(DbwOrder.created_at >= date_from)
        if date_to:
            query = query.where(DbwOrder.created_at <= date_to)
        if delivery_type:
            query = query.where(DbwOrder.delivery_type == delivery_type)
        query = query.order_by(DbwOrder.created_at.desc()).limit(limit).offset(offset)
